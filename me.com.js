...
(function () {
	...
	/* Modules */
	var app = angular.module('MEapp', ['MEcontrollers']);


})();
(function () {

	var numMajorEvents = 0;
	var raiseGAEvent = function(category, action, label, value, raiseConditional) {
	    if(ga && (raiseConditional === undefined || raiseConditional)) {
	        ga('send', 'event', category, label + ' ' + action, value, undefined, {'nonInteraction': 1});

	        if(category === GALayer.category) {
	            numMajorEvents++;
	            if(numMajorEvents === 4) {
	                angular.element(document.getElementById('ctrl_host')).scope().showTour();
	            }
	            if(numMajorEvents % 5 == 0) {
	                angular.element(document.getElementById('ctrl_host')).scope().showAds();
	            }
	        }
	    }
	}
	...
	var GASocial = { category: "social", action: { follow: "follow", share: "share" }, label: { network: "network" }, 
                value: { facebook: "facebook", twitter: "twitter", gplus: "gplus" } };
	var GAScreen = { category: "screen_message", action: {auto:"auto", click:"click"}, label: "message", 
	                value: { mobile: "mobile", oldbrowser: "old_browser/safari", fake_timebar: "fake_timebar" } };
	var GALayer = { category: "layer", action: {enable:"enable", disable:"disable"}, label: {layer:"layer", misc:"misc"}, 
	                value: { layers: ["temperature", "precipitation", "wind", "cloud", "pressure", "storm", "social", "vor"], picker: "picker", _3D: "3D", lighting: "lighting"} };
	var GAPopup = { category: "popups", action: "click", label: "popup", 
	                value: { share: "share", settings: "settings", legend: "legend", embed: "embed"} };
	var GASettings = { category: "settings", action: {auto:"auto", click:"click"}, label: { langauage: "language", metric_units: "metric_units" }};
	var GAPOI = { category: "POI", action: {auto:"auto", click:"click"}, label: { search: "search", poi: "poi", locate_me: "locate_me", meteo24: "meteo24" },
	              value: {start:"start", success: "success", error: "error", na: "n/a"}};
	var GATour = { category: "Tour", action: {auto:"auto", click:"click"}, label: { hard_close: "hard_close", next: "next", prev: "prev", soft_close: "soft_close" }};
	var GAMisc = { category: "Misc", action: {click:"click"}, label: { start_playback: "start_playback", close_ads: "close_ads" }};


	/* Modules */
	var app = angular.module('MEcontrollers', ['tmh.dynamicLocale', 'pascalprecht.translate', 'ngCookies', 'angularSpinner', 'ngAnimate'])
	.filter('clamp_temperature', function() {
	  return function(tempStr) {
	    var temperature = Math.round(parseFloat(tempStr));
	    return Math.min( Math.max(temperature, -64), 64).toString();
	  };    
	})
	.filter('temperature', function() {
	  return function(tempStr, scope) {
	    var temperature = parseFloat(tempStr);
	    if(scope.isUnitSystemMetric === "1") {
	...
	var meConfig = {};
	var meteoEarth = {};
	var feedBaseUrl = 'http://meteoearth.weatherpro.meteogroup.de/weatherpro/';
	var availableLanguages = [   {code: "en", name: 'English'},
	...
	var availableLanguagesBroadcast = [ "en", "de", "es", "nl" ];
	var availableLanguagesMobile = [ "en", "de" ];
	var availableLanguagesNav = [ "en", "de" ];
	var availableLanguagesBLog = [ "en", "de" ];
	var availableMeteo24POIUrls = [
	    { lang: "en", url: "http://www.weathercast.co.uk/world-weather/city-weather/city/{{_country}}X{{_city}}/{{_name}}.html" },
	    { lang: "de", url: "http://www.wetter24.de/wetter/{{_name}}/{{_country}}X{{_city}}.html" },
	    { lang: "fr", url: "http://www.meteo24.fr/fr/home/meteo-france-et-europe/temps-dans-le-monde/temps-des-villes/ville/{{_country}}X{{_city}}/{{_name}}.html" },
	    { lang: "es", url: "http://www.tiempohoy.es/es/home/tiempo/tiempo-mundo/tiempo-ciudad/ciudad/{{_country}}X{{_city}}/{{_name}}.html" },
	    { lang: "it", url: "http://www.iltempo24.it/it/home/tempo/il-tempo-nel-mondo/localita-del-servizio-meteo/citta/{{_country}}X{{_city}}/{{_name}}.html" },
	    { lang: "nl", url: "http://www.weer.nl/wereldweer/actueel-en-verwacht-weer/stad/{{_country}}X{{_city}}/{{_name}}.html" },
	    { lang: "pl", url: "http://www.mojapogoda.com/pogoda-na-swiecie/pogoda-lokalna/miasto/{{_country}}X{{_city}}/{{_name}}.html" },
	    { lang: "sv", url: "http://www.mittvaeder.se/se/home/vaeder/vaerlden/stadsvaeder/stad/{{_country}}X{{_city}}/{{_name}}.html" }
	];
	var codeTranslations = [ 'you_could_not_be_located', 'myLocation', 'search_empty', 'download_failed', 'browser_alternative', 'mobile_unsupported', 'forecast_feature', 'layer_Footer_Info', "layer_Temperature", "layer_Precipitation",	"layer_Wind", "layer_CloudCover", "layer_Isobars", "layer_Temperature_Info", "layer_Precipitation_Info", "layer_Wind_Info", "layer_CloudCover_Info", "layer_Isobars_Info", "share_desc", "layer_TropicalStorm", "layer_TropicalCyclone_Info", "TropicalDepression", "TropicalStorm", "Hurricane", "Cyclone", "Typhoon" ];
	var text = {};
	var isPickerDragged = false;
	var pickerPosition = {left: 0.0, bottom: 0.0};
	var pinHeight = 10;

	// facebook share stuff
	window.fbAsyncInit = function() {
	    FB.init({
	      appId      : '895933663754833',
	      xfbml      : true,
	      version    : 'v2.0'
	    });
	};

	var loadFacebookSDK = function(d, s, id, lang) {
	 var js, fjs = d.getElementsByTagName(s)[0];
	 if (d.getElementById(id)) {return;}
	 js = d.createElement(s); js.id = id;
	 var fblang = lang + "_" + lang.toUpperCase();
	 js.src = "//connect.facebook.net/" + fblang + "/sdk.js";
	 fjs.parentNode.insertBefore(js, fjs);
	};

	app.config(function ($translateProvider, tmhDynamicLocaleProvider, $compileProvider) {
	    tmhDynamicLocaleProvider.localeLocationPattern('l10n/ngLocale/angular-locale_{{locale}}.js');
	    tmhDynamicLocaleProvider.useCookieStorage();

	    $compileProvider.imgSrcSanitizationWhitelist(/^\s*(http|https|blob|data):/);
	    
	    var availableLangKeys = availableLanguages.map(function(lang) { return lang.code; });
		$translateProvider
			.useStaticFilesLoader({
				prefix: 'l10n/',
				suffix: '.json'
			})
			.registerAvailableLanguageKeys(availableLangKeys)
			.fallbackLanguage('en')
	        .useLocalStorage()
			.determinePreferredLanguage(function () {
	            var usersLang =   window.navigator.language || window.navigator.browserLanguage ||
	                              window.navigator.systemLanguage || window.navigator.userLanguage;
	            usersLang = usersLang.substring(0, 2);

	            /*var botLang = getUrlVars()["hl"];
	            if(botLang) { 
	                usersLang = botLang;
	            }*/

	            if(availableLangKeys.indexOf(usersLang) >= 0 ) {
	                return usersLang;
	            }
	            return 'en';
	        });
	...
	app.directive('uiTour', ['$timeout', '$parse', function($timeout, $parse){
	  return {
	    link: function($scope, $element, $attributes) {
	      ...
	      $scope.$watch($attributes.uiTour, function(newVal, oldVal){
	      	// show tour only in desktop mode
	        if (angular.isNumber(newVal) && $(window).width() > 1015) {
	          showStep(newVal);
	        } else {
	        ...
	        if (stepNumber && elm.length) {
	          ...
	          $timeout(function(){
	            ...
	            if(!!target.offsetParent) {
	                offset.top += target.offsetParent.offsetTop;
	                offset.left += target.offsetParent.offsetLeft;
	            }

	            if (elm.attr('overlay') !== undefined) {
	              $('.tour-overlay').addClass('active').css({
	                marginLeft: offset.left + target.offsetWidth / 2 - 250,
	                marginTop: offset.top + target.offsetHeight / 2 - 250
	              }).addClass('in');
	            } else {
	...
	/* Controllers */
	app.controller('MEview', function ($scope, $sce, $http, $translate, $rootScope, $timeout, $interval, $interpolate, usSpinnerService, tmhDynamicLocale) {

		$scope.layerButtonClass = [];
		$scope.showStormList = false;
		$scope.currentTourStep = 0;

		if(typeof(Storage) !== "undefined") {
	        $scope.isUnitSystemMetric = localStorage.getItem("isUnitSystemMetric") || "1";
	    }

		function ResolutionRangeValue(initialValue) {
	        ...
	        this.__defineSetter__("val", function (valStr) {
	            innerValue = parseInt(valStr);
	            $scope.generateCodeConditionally();
	            if(meteoEarth.onCanvasResize) {
	                $timeout( function() {
	                    meteoEarth.onCanvasResize();
	                }, 100 );
	            }
	        });
	    }
	    
	    function TimeRangeValue(initialValue) {
	        var innerValue = initialValue;
	        ...
	        this.__defineSetter__("val", function (valStr) {
	            ...
	            // single place to submit restricted time index to engine
	            var timePart = innerValue / 1000.0;
	            
	            var dataStart = $scope.streamingTimes.startDateTime * 1000;
	            var dataEnd = $scope.streamingTimes.endDateTime * 1000;
	            if(dataEnd > 0) {
	                var minDT = Math.max(Date.now(), dataStart);                       // do not show data before now
	                var maxDT = Math.min(minDT + 24 * 60 * 60 * 1000, dataEnd);        // limit forecast to exact 24 hours
	                var timeSpan = (dataEnd - dataStart);
	                
	                var scale = (maxDT - minDT) / timeSpan;
	                var offset = (minDT - dataStart) / timeSpan;

	                timePart = timePart * scale + offset;

	                var nextDay = new Date(minDT + 24 * 60 * 60 * 1000);
	                nextDay.setHours(0);
	                nextDay.setMinutes(0);
	                nextDay.setSeconds(0);
	                
	                var timebar = $("#timebar");
	                var sliderPosition = timebar.position();
	                var sliderWidth = timebar.width();
	                $scope.timebarDays = [
	                    {
	                        time: nextDay,
	                        style: { left: (sliderPosition.left + (sliderWidth * (nextDay.getTime() - minDT) / (maxDT- minDT))) + "px" }
	                    }
	                ];
	            }
	            meteoEarth.setTimeIndex(timePart);
	        });
	    }

	    var shouldShowFullscreenMessage = function(compatibility) {
		    return compatibility && (!compatibility.isCompatible);
		};
	    ...
	    var StormViewModel = function() {
	        ...
	        this.update = function(stormName, stormBasin, stormSample) {
			    this.lon = stormSample.longitude;
			    this.lat = stormSample.latitude;
	            this.name = stormName;
			    this.updateDescription(stormBasin, stormSample);
			    this.updateMetrics(stormSample);
	        };

	        this.updateDescription = function(stormBasin, stormSample)
		    {
		        var stage = stormSample.stormtype;

		        if( stage !== "HU" )
		        {
		            this.desc = (stage === "TD") ? text.TropicalDepression : text.TropicalStorm;
	                return;
		        }

		        var designation = text.Hurricane;
		    
		        if( stormBasin != null )
		        {
			        var basin = stormBasin[0];
		            switch (basin)
		            {
		                case 'A':   // Arabian Sea
		                case 'B':   // Bay of Bengal
		                case 'P':   // South Pacific
		                case 'S':   // South Indian
		                    designation = text.Cyclone;
		                    break;
		                case 'W':   // West Pacific
		                    designation = text.Typhoon;
		                    break;
		                case 'C':   // Central Pacific
		                case 'E':   // East Pacific
		                case 'L':   // Atlantic
		                default:
		                    // hurricane is the default
		                    break;
		            }
		        }

	            var that = this;
	            $translate('HurricaneCategory', { designation: designation, category: stormSample.category }).then(function (translation) {
	                that.desc = translation;
	            });
		    };
	        
	        var knotsToSpeedUnit = function(knots, isMetric) {
	            if(isMetric) {
	                return knots * 1.852;
	            } else {
	        ...
	        this.updateMetrics = function(stormSample)
		    {
	            var isMetric = ($scope.isUnitSystemMetric === "1");

		        this.metrics.windspeed = knotsToSpeedUnit(stormSample.wind, isMetric).toFixed(0);
		        this.metrics.gustspeed = knotsToSpeedUnit(stormSample.gust, isMetric).toFixed(0);
		        this.metrics.stormspeed = knotsToSpeedUnit(stormSample.speed, isMetric).toFixed(0);

		        this.metrics.windunit = isMetric ? "km/h" : "mph";
		        this.metrics.speedunit = this.metrics.windunit;
	        };
	    };

		var conditionalyCreateMEView = function(compatibility) {

		    if(shouldShowFullscreenMessage(compatibility)) {
		        $timeout(function () { // angularjs timeout call is needed here since we need to get back to angular scope
                    $scope.compatibility = compatibility;
                    $scope.updateLocalizations();
		        });
		    } else {

			meConfig.div = document.getElementById('MEview');
			meConfig.src = "";
			meConfig.isShareUrlEnabled = true;
			meConfig.isShareUrlTimeEnabled = true;
			meConfig.viewSettings = {
				longitude: 4.0,
				latitude: 30.0,
				zoom: 2.5,
				is3DEnabled: true,
				isLightingEnabled: true,
				initialRotationSpeed: 20.0
			};
			meConfig.pois = [];
			meConfig.StormViewModel = StormViewModel;
			meConfig.enableInitialLayersPromise = true;

			meConfig.onInitialized = function() {

				// enable wind & social after seven seconds of inactive user
				if(meConfig.enableInitialLayersPromise) {
					meConfig.enableInitialLayersPromise = window.setTimeout(function() {
						// wind
						meteoEarth.setLayerEnabled(2, true);
						//social
						meteoEarth.setLayerEnabled(6, true);
					}, 7000);
				}

				// add active class to all initialy enabled layers
				for (var i = 0; i <= 7; i++) {
					if (meteoEarth.isLayerEnabled(i)) {
						$scope.layerButtonClass[i] = 'active';
					}
				};

				$timeout(function () {
					$scope.stormList = meteoEarth.tropicalStormMgr.stormListData;
				});
			};

			meConfig.getLayerElement = function(layerId, isEmbedElement) {
				return document.getElementById((isEmbedElement ? 'em' : '') + 'layer' + layerId);
			};
			meConfig.isAnyLayerEnabled = function() {
				for (n = 0; n < 5; n++) {
					if( meConfig.getLayerElement(n).className.indexOf("active") >= 0 ) {
						return true;
					}
				}
				return false;
			};
			meConfig.onLayerEnabledChanged = function(layerIndex, isEnabled) {
	            if(meConfig.enableInitialLayersPromise) {
	                if(!isNaN(meConfig.enableInitialLayersPromise)) {
	                    window.clearTimeout(meConfig.enableInitialLayersPromise);
	                }
	                meConfig.enableInitialLayersPromise = undefined;
	            }
	              
	            meConfig.setButtonClass(layerIndex, isEnabled);
	            $scope.generateCodeConditionally();

	            meConfig.checkPickerInfoAvailability();
          	};
          	meConfig.onSocialWeatherEnabled = function(isEnabled) {
	              meConfig.setButtonClass(32, isEnabled);
	          };
          	meConfig.onVOREnabled = function(isEnabled) {
	              meConfig.setButtonClass(33, isEnabled);
	          };
          	meConfig.onTropicalStormEnabled = function(isEnabled) {
	              meConfig.setButtonClass(5, isEnabled);
	              $scope.generateCodeConditionally();

	              meConfig.checkPickerInfoAvailability();
	              
	              if(isEnabled && !meConfig.isAnyLayerEnabled()) {
	                  // automatically enable wind to visualize storm, if nothing else was activated
	                  meteoEarth.setLayerEnabled(2, true);
	              }

	              $timeout(function () {
	                  $scope.showStormList = isEnabled;
	              });
	          };

          	meConfig.on3DEnabledChanged = function(isEnabled) {
         		
          		var button1 = document.getElementById('button_3d');
          		var button2 = document.getElementById('em_button_3d');
          		//var embutton = document.getElementById('embutton_3d');

          		button1.className = button1.dataset.basename + (isEnabled ? "_on" : "_off");
          		button2.className = button2.dataset.basename + (isEnabled ? "_on" : "_off");
          		//embutton.className = button.dataset.basename + (isEnabled ? "_on" : "_off");
				$scope.generateCodeConditionally();
	      	};
	      	meConfig.onLightingEnabledChanged = function(isEnabled) {
	            var button1 = document.getElementById('button_lighting');
	            var button2 = document.getElementById('em_button_lighting');
          		//var embutton = document.getElementById('embutton_lighting');
          		button1.className = button1.dataset.basename + (isEnabled ? "_on" : "_off");
          		button2.className = button2.dataset.basename + (isEnabled ? "_on" : "_off");
          		//embutton.className = button.dataset.basename + (isEnabled ? "_on" : "_off");
	              $scope.generateCodeConditionally();
	          ...
	          meConfig.onIsLoadingDataChanged = function(isLoadingData) {
	              if(isLoadingData) {
	                usSpinnerService.spin('loadingDataSpinner');
	              } else {
	                usSpinnerService.stop('loadingDataSpinner');
	              }
	          };

			meConfig.checkPickerInfoAvailability = function() {
				$timeout(function () {
					$scope.isPickerLayerEnabled = meConfig.isAnyLayerEnabled();
					$scope.isInfoLayerEnabled = $scope.isPickerLayerEnabled || meConfig.viewSettings.isTropicalStormEnabled;

					$scope.showPicker = $scope.isPickerLayerEnabled && $scope.showPicker;
				}, 100);
			};

			meConfig.onPickerUpdate = function(pickerStyle, pickerData) {
              $timeout(function () {
                  if(!isPickerDragged) {
                      pickerPosition.left = pickerStyle.left;
                      pickerPosition.bottom = pickerStyle.bottom;
                      pickerStyle.bottom += pinHeight;
                      $scope.pickerStyle = pickerStyle;
                  }
                  
                  var texts = [];
                  var isMetric = ($scope.isUnitSystemMetric === "1");
                  var newPickerTexts = [];
                  if(pickerData.temperature) {
                      var temperature = (isMetric ? pickerData.temperature : pickerData.temperature * 1.8 + 32.0);
                      newPickerTexts.push( temperature.toFixed(1) + (isMetric ? " °C" : " °F") );
                  }
                  if(pickerData.precipitation) {
                      var precipitation = (isMetric ? pickerData.precipitation.toFixed(1) : (pickerData.precipitation * 0.0393700787).toFixed(2) );
                      newPickerTexts.push( precipitation + (isMetric ? " mm" : " in") );
                  }
                  if(pickerData.cloudCover) {
                      newPickerTexts.push( (pickerData.cloudCover * 100.0).toFixed(0) + " %" );
                  }
                  if(pickerData.wind) {
                      var wind = pickerData.wind * (isMetric ? 3.6 : 2.23693629);
                      newPickerTexts.push( wind.toFixed(0) + (isMetric ? " km/h" : " mph") );
                  }
                  if(pickerData.pressure) {
                      var pressure = (isMetric ? pickerData.pressure.toFixed(1) : (pickerData.pressure * 0.0295299830714).toFixed(2) );
                      newPickerTexts.push( pressure + (isMetric ? " hPa" : " inHg") );
                  }
                  $scope.pickerText = newPickerTexts.length === 0 ? " - " : 
                    newPickerTexts.reduce(function(result, part) { return result + ((result.length > 0) ? "\n" : "" ) + part; }, "");
              });
          };

          meConfig.addOrUpdatePOI = function(poi) {
              var existingPOI = document._.find(meConfig.pois, function(p) { return p.id === poi.id; });
              if(existingPOI) {
                  meConfig.pois[document._.indexOf(meConfig.pois, existingPOI)] = poi;
              } else {
                  meConfig.pois.push(poi);
              }
          };
          
          meConfig.removePOI = function(poiId) {
				var existingPOI = document._.find(meConfig.pois, function(p) { return p.id === poiId; });
				if(existingPOI) {
				  meConfig.pois.splice(document._.indexOf(meConfig.pois, existingPOI), 1);
				}
			};

			meConfig.onPOIClick = function(poiKey) {

				var socialWeatherEvent = meteoEarth.socialWeatherManager.swEvents[poiKey];
				if(socialWeatherEvent) {
				  	meConfig.openSocialWeatherEvent(socialWeatherEvent);
				} else {
					var poi = meteoEarth.poiManager.getPOIForKey(poiKey);
					var newPOI = { 
					    name: poiKey,
					    lon: poi.lon, 
					    lat: poi.lat, 
					...
					var vorTeamData = meteoEarth.vorMgr.teamData[poiKey];
					if(vorTeamData) {
					    newPOI.name = vorTeamData.name;
					    newPOI.lon = vorTeamData.lon;
					    newPOI.lat = vorTeamData.lat;
					}
					meConfig.addOrUpdatePOI(newPOI);
				}
			};
			
			meConfig.openSocialWeatherEvent = function(socialWeatherEvent) {
                
                var point3D = meteoEarth.geoToScreenCoords(socialWeatherEvent.lon, socialWeatherEvent.lat);    
                var isVisible = point3D[2] > 0.0;

                if(isVisible) {
                    // Pause video
                    $timeout(function () {
                        window.document.getElementById("social_poi_video_player").pause();
                    });

                    var divRect = meConfig.div.getBoundingClientRect();
                    
                    $timeout(function () {
                        $scope.showSocialPoi = true;

                        var needsBottomClass = point3D[1] < meConfig.div.clientHeight * 0.5;
                         $scope.social_poi_class = needsBottomClass ? "social_poi_bottom" : "social_poi";

                        $scope.socialWeatherPOIContent = { 
                            media_url: socialWeatherEvent.content != "" ? $sce.trustAsResourceUrl($scope.blackPixelBlob) : "",
                            text: socialWeatherEvent.text,
                            poi_style: {
                                left: point3D[0], 
                                top: needsBottomClass ? point3D[1] + 10 : undefined,
                                bottom: needsBottomClass ? undefined : divRect.height - point3D[1] + 10, 
                                opacity: (point3D[2] * 10.0)
                            },
                            coordinates: {lon: socialWeatherEvent.lon, lat: socialWeatherEvent.lat},
                            user_name: socialWeatherEvent.userName,
                            profile_image_url: $sce.trustAsResourceUrl($scope.blackPixelBlob),
                            place_name: socialWeatherEvent.placeName,
                            source: socialWeatherEvent.source,
                            isVideo: socialWeatherEvent.content.indexOf(".mp4") > -1,
                            profileUrl: (socialWeatherEvent.source == "Twitter") ? "https://twitter.com/" + socialWeatherEvent.userName : "http://instagram.com/" + socialWeatherEvent.userName                     
                        };

                        $timeout(function () {
                            if(socialWeatherEvent.content != "" && socialWeatherEvent.content.indexOf(".mp4") == -1)
                            {
                                usSpinnerService.spin('socialPOIMainLoadSpinner');
                            } else {
                                usSpinnerService.stop('socialPOIMainLoadSpinner');
                            }

                            usSpinnerService.spin('socialPOIUserLoadSpinner');

                            $scope.socialWeatherPOIContent.profile_image_url = socialWeatherEvent.userImageUrl;
                            if(socialWeatherEvent.content != "") {                    
                                $scope.socialWeatherPOIContent.media_url = $sce.trustAsResourceUrl(socialWeatherEvent.content);
                            }
                        }, 100);
                    });     
                }
            };
            meConfig.onUpdate = function(deltaTime) {

            	if($scope.isPlayBackEnabled) {
                    $timeout(function () {
                        $scope.timeIndex.val += 2; // not framerate yet dependent because fractions are not allowed here
                        if($scope.timeIndex.val > 1000 ){
                            $scope.timeIndex.val = 0;
                        }
                    });
                }

                // update clock to show correct time (+update every new full minute)
                $scope.streamingTimes = meteoEarth.getStreamingTimes();
                var now = $scope.streamingTimes.currentDateTime * 1000;
                var realNow = Date.now();
                var currentClockUpdateMinute = Math.floor(realNow / 60000);
                if(now == 0) {
                    now = realNow;
                }
                if($scope.realTimeClock !== now || $scope.lastClockUpdateMinute !== currentClockUpdateMinute){
                    $scope.realTimeClock = now;
                    $scope.timeIndex.val = $scope.timeIndex.val; // force engine update
                    $scope.lastClockUpdateMinute = currentClockUpdateMinute;
                }

                if($scope.showSocialPoi) {
                    var divRect = meConfig.div.getBoundingClientRect();
                    var point3D = meteoEarth.geoToScreenCoords(
                        $scope.socialWeatherPOIContent.coordinates.lon, 
                        $scope.socialWeatherPOIContent.coordinates.lat);
                    var needsBottomClass = point3D[1] < meConfig.div.clientHeight * 0.5;

                    $timeout(function () {
                        $scope.social_poi_class = needsBottomClass ? "social_poi_bottom" : "social_poi";
                        $scope.socialWeatherPOIContent.poi_style = {
                            left: point3D[0], 
                            top: needsBottomClass ? point3D[1] + 10 : undefined,
                            bottom: needsBottomClass ? undefined : divRect.height - point3D[1] + 10, 
                            opacity: (point3D[2] * 10.0)};                    
                    });
                    $scope.isSocialPoiVisible = (point3D[2] * 10.0) > 0.01;
                }
            };

            meConfig.setButtonClass = function(layerIndex, isEnabled) {
            	if (isEnabled) {
            		$scope.layerButtonClass[layerIndex] = 'active';
				} else {
					$scope.layerButtonClass[layerIndex] = '';
				};
				return isEnabled;
			};

			meConfig.locationSuccess = function(position) {

				$timeout(function () { usSpinnerService.stop('locateMeSpinner'); });
				meConfig.pois = [{name:text.myLocation, lon: position.coords.longitude, lat:position.coords.latitude, style: "poi", pinHeight: pinHeight, id: "myLocation"}];
				meteoEarth.centerOnLocation(position.coords.longitude, position.coords.latitude);

				raiseGAEvent(GAPOI.category, GAPOI.action.auto, GAPOI.label.locate_me, GAPOI.value.success);
			};
			meConfig.locationError = function(error) {
				$timeout(function () { usSpinnerService.stop('locateMeSpinner'); });
				alert(text.you_could_not_be_located);

				raiseGAEvent(GAPOI.category, GAPOI.action.auto, GAPOI.label.locate_me, GAPOI.value.error);
			};

			meteoEarth = me_createView(meConfig);

		    }
	    };


	    me_testCompatibility(conditionalyCreateMEView, true);


	    // Picker Handling
	    {
	        var getEventPos = function(element, evt) {
	        ...
	        var startPickerDragging =  function(evt) {
	            var newMousePos = getEventPos(meConfig.div, evt);
	            pickerStartPos = newMousePos;
	            isPickerDragged = true;
	            evt.preventDefault();
	        };
	        
	        var performPickerDragging = function(evt) {
	            if(!isPickerDragged) return;
	            evt.preventDefault();
	            var newMousePos = getEventPos(meConfig.div, evt);
	            var deltaX = newMousePos.x - pickerStartPos.x;
	            var deltaY = newMousePos.y - pickerStartPos.y;

	            var divRect = meConfig.div.getBoundingClientRect();
	            var newPickerX = pickerPosition.left + deltaX;
	            var newPickerY = divRect.height - pickerPosition.bottom + deltaY;
	            var geoCoords = meteoEarth.screenToGeoCoords(newPickerX, newPickerY);

	            if(geoCoords[2] < 0.0 ) {
	                isPickerDragged = false;
	                $timeout(function () {
	                    $scope.showPicker = false;
	                });
	                return;
	            }
	            meteoEarth.setPickerPosition(geoCoords[0], geoCoords[1]);

	            $timeout(function () {
	                $scope.pickerStyle.left = newPickerX;
	                $scope.pickerStyle.bottom = divRect.height - newPickerY + pinHeight;
	            });
	        };
	        
	        var stopPickerDragging = function(evt) {
	            isPickerDragged = false;
	        };

	        document.getElementById('picker').addEventListener( "mousedown", startPickerDragging);
	        document.getElementById('picker').addEventListener( "touchstart", startPickerDragging);
	        document.addEventListener( "mousemove", performPickerDragging);
	        document.addEventListener( "touchmove", performPickerDragging);
	        document.addEventListener( "mouseup", stopPickerDragging);
	        document.addEventListener( "touchend", stopPickerDragging);
	    }

	    $scope.embedWidth = new ResolutionRangeValue(420);
	    $scope.embedHeight = new ResolutionRangeValue(420);
	    $scope.showPicker = false;
		$scope.languages = availableLanguages;
	    $scope.navLang = availableLanguagesNav[0];
	    $scope.blogLang = availableLanguagesBLog[0];
	    $scope.timeIndex = new TimeRangeValue(0);

	    $scope.changeLang = function () {
			$translate.use($scope.selectedItem.code);
		};

	    $scope.updatePOIUrl = function() {
	        if(!$scope.selectedPOI) return;

	        var foundPOIUrl = availableMeteo24POIUrls.filter(function(poiUrl, index){ return poiUrl.lang == $scope.currentLang; });
	        var baseMeteo24Url = $interpolate(foundPOIUrl.length > 0 ? foundPOIUrl[0].url : availableMeteo24POIUrls[0].url);
	        $scope.meteo24POIUrl = baseMeteo24Url($scope.selectedPOI);
	    }
	    ...
	    $rootScope.$on('$translateChangeSuccess', function () {
			...
			$scope.selectedItem = $scope.languages.filter( function(item, index) { return item.code == $scope.currentLang; })[0];
			$translate(codeTranslations).then(function (translations) {
				text = translations;
				$scope.updateLocalizations();
			});

	        $scope.mobileLang = availableLanguagesMobile.indexOf($scope.currentLang) != -1 ? $scope.currentLang : availableLanguagesMobile[0];
	        $scope.broadcastLang = availableLanguagesBroadcast.indexOf($scope.currentLang) != -1 ? $scope.currentLang : availableLanguagesBroadcast[0];
	        $scope.navLang = availableLanguagesNav.indexOf($scope.currentLang) != -1 ? $scope.currentLang : availableLanguagesNav[0];
	        $scope.blogLang = availableLanguagesBLog.indexOf($scope.currentLang) != -1 ? $scope.currentLang : availableLanguagesBLog[0];

	        var foundMeteo24Url = availableMeteo24POIUrls.filter(function(poiUrl, index){ return poiUrl.lang == $scope.currentLang; });
	        var baseMeteo24Url = foundMeteo24Url.length > 0 ? foundMeteo24Url[0].url : availableMeteo24POIUrls[0].url;
	        $scope.meteo24Url = baseMeteo24Url.substring(0, baseMeteo24Url.indexOf("/", 7));

	        $scope.updatePOIUrl();

	        $scope.hide_everyhing = false;

	        //$scope.langChangeAction = ($scope.langChangeAction === undefined) ? GASettings.action.auto : GASettings.action.click;
	        raiseGAEvent(GASettings.category, $scope.langChangeAction, GASettings.label.langauage, $scope.currentLang);
		});

		$scope.disableOverlays = function () {

			// close social poi only on click, not on drag
			// http://stackoverflow.com/questions/21850989/how-to-detect-click-but-not-drag-using-jquery
		    var isDragging = false;
		    $(".earthContainer, .navbar-left")
		    .mousedown(function() {
		    ...
		    .mouseup(function() {
		        ...
		        $(window).unbind("mousemove");
		        if (!wasDragging) {
					$scope.closeSocialPOI();
		        }
		    });

			$scope.showPOISearchResults = false;
			$scope.showShareTooltipMenu = false;
			$scope.showSettingsTooltipMenu = false;
			$scope.showInfoTooltipMenu = false;
		}

	    $scope.updateLocalizations = function() {

	        if(meConfig.pois) {
	            var myLocationPOIs = meConfig.pois.filter( function(poi, index) { return poi.id === "myLocation" });
	            if(myLocationPOIs.length > 0 ) {
	                myLocationPOIs[0].name = text.myLocation;
	            }
	        }

	        $scope.raiseGAScreenEvent = ($scope.raiseGAScreenEvent === undefined) ? true : false;
	        $scope.showBrowsers = false;
	        $scope.showFullScreenMessage = false;
	        if(shouldShowFullscreenMessage($scope.compatibility)) {
	            if($scope.compatibility.isMobile) {
	                $scope.fullscreenMessage = text.mobile_unsupported;
	                raiseGAEvent(GAScreen.category, GAScreen.action.auto, GAScreen.label, GAScreen.value.mobile, $scope.raiseGAScreenEvent);
	            } else {
	                $scope.fullscreenMessage = text.browser_alternative;
	                $scope.showBrowsers = true;
	                raiseGAEvent(GAScreen.category, GAScreen.action.auto, GAScreen.label, GAScreen.value.oldbrowser, $scope.raiseGAScreenEvent);
	            }
	            $scope.showFullScreenMessage = true;
	        }
	    };

	    $scope.locateMe = function() {
			$timeout(function () { usSpinnerService.spin('locateMeSpinner'); });
			navigator.geolocation.getCurrentPosition(meConfig.locationSuccess, meConfig.locationError, {timeout: 10000});
		};
		...
		$scope.showTour = function() {
	        if( localStorage.getItem("showTour") !== "false" ) {
	            ...
	            raiseGAEvent(GATour.category, GATour.action.auto, GATour.label.next, $scope.currentTourStep);
	        }
	    };
	    $scope.hideTour = function() {
	        $scope.currentTourStep = false;
	        raiseGAEvent(GATour.category, GATour.action.click, GATour.label.soft_close, $scope.currentTourStep);
	    };
	    $scope.closeTour = function() {
	        $scope.currentTourStep = false;
	        localStorage.setItem("showTour", "false");
	        raiseGAEvent(GATour.category, GATour.action.click, GATour.label.hard_close, $scope.currentTourStep);
	    };
	    $scope.prevTip = function() {
	        raiseGAEvent(GATour.category, GATour.action.click, GATour.label.prev, $scope.currentTourStep);
	        $scope.currentTourStep = $scope.currentTourStep - 1;
	    };
	    $scope.nextTip = function() {
	        raiseGAEvent(GATour.category, GATour.action.click, GATour.label.next, $scope.currentTourStep);
	        $scope.currentTourStep = $scope.currentTourStep + 1;
	    };
	    ...
	    $scope.searchPOI = function(searchString) {
	        $scope.searchString = trim(searchString);
	        if ( $scope.searchString === "" || $scope.searchString.length < 3 ) {
	            return;
	        }

	        raiseGAEvent(GAPOI.category, GAPOI.action.click, GAPOI.label.search, searchString);
        	usSpinnerService.spin('poiSearchSpinner');

	        $http({method: 'GET', url: feedBaseUrl + 'SearchFeed.php?lang=' + $scope.currentLang +'&search=' + $scope.searchString, searchString: $scope.searchString}).
	        success(function(data, status, headers, config) {
	            if(config.searchString !== $scope.searchString ) return;

	            usSpinnerService.stop('poiSearchSpinner');

	            var x2js = new meteoEarth.X2JS();
	            var searchResult = x2js.xml_str2json(data);

	            if(!searchResult.result.cities && !searchResult.result.pois) {
	                $scope.searchResult = { status: [ text.search_empty ] };
	            } else {
	                var result = { // ensure results come in arrays
	                    cities: (!searchResult.result.cities ? [] : (searchResult.result.cities.city.length ? searchResult.result.cities.city : [searchResult.result.cities.city])),
	                    pois: (!searchResult.result.pois ? [] : (searchResult.result.pois.poi.length ? searchResult.result.pois.poi : [searchResult.result.pois.poi]))
	                };
	                
	                $scope.searchResult = result;
	            }
	            $scope.showPOISearchResults = true;
	            console.log($scope.searchResult.pois);
	        }).
	        error(function(data, status, headers, config) {
	            if(config.searchString !== $scope.searchString ) return;
	            usSpinnerService.stop('poiSearchSpinner');
	            $scope.searchResult = { status: [ text.download_failed ] };
	            $scope.showPOISearchResults = true;
	        });
	    };

	    // poi search
	    $scope.poiSearchPromise = null;
	    $scope.$watch('poiSearchText', function(newValue, oldValue) {
	        if(!newValue) return;
	        if($scope.poiSearchPromise ) $timeout.cancel($scope.poiSearchPromise);
	        usSpinnerService.stop('poiSearchSpinner');

	        $scope.poiSearchPromise = $timeout(function () {
	            $scope.searchPOI(newValue);
	        }, 500);
	    });

	    $scope.$watch('showPOISearchResults', function(newValue, oldValue) {
	        //$scope.poiForecastOffsetClass = newValue ? 'offset' : '';
	        if(newValue) {
	            $scope.hideAds();
	        }
	    });
	    ...
	    if(typeof(Storage) !== "undefined") {
	        $scope.$watch('isUnitSystemMetric', function(newValue, oldValue) {
	            localStorage.setItem("isUnitSystemMetric", newValue);
	            if(newValue !== oldValue) {
	                raiseGAEvent(GASettings.category, GASettings.action.click, GASettings.label.metric_units, newValue);
	            }
	        });
	    }

	    $scope.openPOI = function(poi) {
	        var lon = parseFloat(poi._longitude);
	        var lat = parseFloat(poi._latitude);
	        $scope.showPOISearchResults = false;
	        meteoEarth.centerOnLocation(lon, lat);
	        meConfig.pois = [{name:poi._name, lon: lon, lat: lat, style: "poi", pinHeight: pinHeight, id: "selectedPOI"}];
	        $scope.selectedPOI = poi;
	        $scope.updatePOIUrl();
	        
	        if ($(window).width() > 1015) {
	        	$scope.openPOIForecast(poi);
	        };

		   raiseGAEvent(GAPOI.category, GAPOI.action.click, GAPOI.label.poi, poi._name);
		};

		$scope.openPOIForecast = function(poi) {
	        var feedUrl = feedBaseUrl + 'WeatherFeed.php?widget=1&lid=' + poi._id;

	        $scope.showPOIForecast = false;
	        $http({method: 'GET', url: feedUrl}).
	        success(function(data, status, headers, config) {

	            var x2js = new meteoEarth.X2JS();
	            var poiForecast = x2js.xml_str2json(data);
	            
	            var numDaysAdded = 0;
	            var today = new Date();
	            today.setHours(0,0,0,0);

	            $scope.forecastResult = { days: poiForecast.WeatherWidgetServiceResponse.forecast.days.day.filter( function(day, index) {
	                var dayDate = Date.parse(day._date);
	                if(today <= dayDate && numDaysAdded < 3 ){                    
	                    numDaysAdded++;
	                    return true;
	                }
	                return false;
	            })};
	            $scope.showPOIForecast = true;
	        }).
	        error(function(data, status, headers, config) {
	            // do nothing, since forecast is optional
	        });
	    };

		$scope.openTooltipShareMenu = function() {

			$scope.showSettingsTooltipMenu = false;
			$scope.showInfoTooltipMenu = false;
			$scope.showShareTooltipMenu = !$scope.showShareTooltipMenu;


	        $scope.showShareButtons = false;
	        usSpinnerService.spin('waitingForSharedImageSpinner');
	       
	        raiseGAEvent(GAPopup.category, GAPopup.action, GAPopup.label, GAPopup.value.share);

	        loadFacebookSDK(document, 'script', 'facebook-jssdk', $scope.currentLang);

	        $scope.shareImage = 'img/me_logo.png';
	        $scope.showShareMenu = true;

	        var logoImg = document.createElement('img');
	        logoImg.onload = function() {

	            // create composed share image (screenshot + datetime + pois + MeteoEarth logo)
	            var dateTimeText = new Date().toUTCString();
	            var shareCanvas = document.createElement('canvas');
	            shareCanvas.width = meteoEarth.canvas.clientWidth;
	            shareCanvas.height = meteoEarth.canvas.clientHeight;
	            var ctx = shareCanvas.getContext("2d");

	            ctx.drawImage(meteoEarth.canvas, 0, 0);
	            
	            // draw POIs
	            var numPois = meConfig.pois ? meConfig.pois.length : 0;
	            for (n = 0; n < numPois; n++) {
	                var poi = meConfig.pois[n];
	                var poiElement = meteoEarth.poiElements[n];

	                if(poiElement.style.opacity > 0.0) {
	                    var rect = poiElement.getBoundingClientRect();

	                    var borderWidth = parseFloat($(poiElement).css("border-width"));
	                    ctx.globalAlpha = $(poiElement).css("opacity");
	                    ctx.fillStyle = $(poiElement).css("background-color");
	                    ctx.fillRect(rect.left, rect.top, rect.width, rect.height);
	  
						ctx.beginPath();
						ctx.lineWidth = borderWidth;
	                    ctx.strokeStyle = $(poiElement).css("border-color");
	                    ctx.rect(rect.left - borderWidth, rect.top - borderWidth, rect.width + borderWidth * 2, rect.height + borderWidth * 2);
	                    ctx.closePath();
						ctx.stroke();

	                    var paddingLeft = parseFloat($(poiElement).css("padding-left"));
	                    var paddingBottom = parseFloat($(poiElement).css("padding-bottom"));
	                    ctx.font = $(poiElement).css("font");
	                    ctx.fillStyle = $(poiElement).css("color");
	                    ctx.fillText(poi.name, rect.left + paddingLeft, rect.top + rect.height - 3 - paddingBottom);
	                    
	                    ctx.beginPath();
	                    ctx.fillStyle = $(poiElement).css("border-color");
	                    ctx.moveTo(rect.left - borderWidth, rect.top + rect.height);
	                    ctx.lineTo(rect.left - borderWidth, rect.top + rect.height + 11);
	                    ctx.lineTo(rect.left - borderWidth + 11, rect.top + rect.height);
	                    ctx.closePath();
	                    ctx.fill();
	                    ...
	                    ctx.lineTo(rect.left, rect.top + rect.height + 8);
	                    ctx.lineTo(rect.left + 8, rect.top + rect.height);
	                    ctx.closePath();
	                    ctx.fill();
	                }
	            }
	            ...
	            var imageScaling = 2.0;
	            ctx.drawImage(logoImg, shareCanvas.width - logoImg.width * imageScaling - 25, shareCanvas.height - logoImg.height * imageScaling - 10, logoImg.width * imageScaling, logoImg.height * imageScaling);
	            
	            ctx.font = "bold 30px Arial";
	            ctx.fillStyle = "#000";
	            ctx.fillText(dateTimeText, 30 + 1, shareCanvas.height - 20 + 1);
	            ctx.fillStyle = "#FFF";
	            ctx.fillText(dateTimeText, 30, shareCanvas.height - 20);

	            var shareImage = shareCanvas.toDataURL("image/jpeg", 0.85);

	            $timeout(function () { // get back to angular scope
	                $scope.shareImage = shareImage;
	                $scope.shareImageUrl = shareImage;
	                $scope.shareURL = window.location.href;

	                $scope.uploadSharedImage(function(imageUrl) {
	                    $scope.shareImageUrl = imageUrl;
	                    $scope.showShareButtons = true;
	                    usSpinnerService.stop('waitingForSharedImageSpinner');
	                });
	            });
	        };
	        logoImg.src = 'img/me_logo.png';
	    };

	    $scope.openTooltipSettingsMenu = function() {

	    	$scope.showShareTooltipMenu = false;
			$scope.showInfoTooltipMenu = false;
		    $scope.showSettingsTooltipMenu = !$scope.showSettingsTooltipMenu;
		}
	    ...
	    $scope.openLegend = function() {
	        ...
	        $scope.showShareTooltipMenu = false;
			$scope.showSettingsTooltipMenu = false;
	        $scope.showInfoTooltipMenu = !$scope.showInfoTooltipMenu;

	        raiseGAEvent(GAPopup.action, GAPopup.action, GAPopup.label, GAPopup.value.legend);

	        var vs = meConfig.viewSettings;
	        var isMetric = ($scope.isUnitSystemMetric === "1");

	        if(vs.isTemperatureEnabled) {
	            $scope.legends.push({
	                title: text.layer_Temperature, 
	                text: text.layer_Temperature_Info,
	                scale: (isMetric ? "img/legend/temp_c.png" : "img/legend/temp_f.png")
	            });
	        }
	        if(vs.isPrecipitationEnabled) {
	            $scope.legends.push({
	                title: text.layer_Precipitation, 
	                text: text.layer_Precipitation_Info,
	                scale: (isMetric ? "img/legend/prec_mm.png" : "img/legend/prec_in.png")
	            });
	        }
	        if(vs.isCloudEnabled) {
	            $scope.legends.push({
	                title: text.layer_CloudCover, 
	                text: text.layer_CloudCover_Info,
	                scale: ""
	            });
	        }
	        if(vs.isWindEnabled) {
	            $scope.legends.push({
	                title: text.layer_Wind, 
	                text: text.layer_Wind_Info,
	                scale: (isMetric ? "img/legend/wind_kmh.png" : "img/legend/wind_mph.png")
	            });
	        }
	        if(vs.isTropicalStormEnabled) {
	            $scope.legends.push({
	                title: text.layer_TropicalStorm, 
	                text: text.layer_TropicalCyclone_Info,
	                scale: ""
	            });
	        }
	        if(vs.isPressureEnabled) {
	            $scope.legends.push({
	                title: text.layer_Isobars, 
	                text: text.layer_Isobars_Info,
	                scale: (isMetric ? "img/legend/iso_hpa.png" : "img/legend/iso_inhg.png")
	            });
	        }

	        $scope.legends.push({title: "", text: text.layer_Footer_Info, scale:""});

	    };

	    $scope.$watch('showPicker', function(newValue, oldValue) {

	        if(!meteoEarth.setPickerPosition) return;

	        meteoEarth.setPickerPosition(meConfig.viewSettings.longitude, meConfig.viewSettings.latitude);
	        meteoEarth.setPickerEnabled(newValue);

	        if(!newValue) {
	            $scope.pickerStyle = {left: -1000, bottom: -1000, opacity: 0.0};
	        }
	        raiseGAEvent(GALayer.category, newValue ? GALayer.action.enable : GALayer.action.disable, GALayer.label.misc, GALayer.value.picker);
	    });

	    $scope.$watch('showEmbedWindow', function(newValue, oldValue) {
	        if(newValue) {
	            $scope.togglePlayback(false);
	            
	            $("#MEview").detach().prependTo('#MEviewHostEmbedded');
	            $("#MEview").addClass("MEviewEmbedded");
	            $("#MEview").removeClass("MEview");
	            $("#mapBuilder").addClass("shown");
	            
	            meConfig.pois = [];
	            $scope.generateCode();
	            
	            meteoEarth.setIsSocialWeatherEnabled(false);
	            
	            raiseGAEvent(GAPopup.action, GAPopup.action, GAPopup.label, GAPopup.value.embed);
	        } else {
	            $("#MEview").detach().prependTo('.earthContainer');
	            $("#MEview").addClass("MEview");
	            $("#MEview").removeClass("MEviewEmbedded");
	            $("#mapBuilder").removeClass("shown");
	        }
	        if(meteoEarth.onCanvasResize) {
	            $timeout( function() {
	                meteoEarth.onCanvasResize();
		...
		var shareServer = 'http://cdn-me.meteogroup.com/upload';
	    $scope.uploadSharedImage = function(callback){
	        var urlParts = $scope.shareURL.split("#");
	        var id = (( (urlParts.length > 1) ? urlParts[1].substr(2) : Date.now().toFixed(0)) + "_" + (Math.random() * 1000000).toFixed(0) )  + ".jpg";
	        var shareData = { id: id, data: $scope.shareImage};
	        $http({method : 'POST', url:shareServer + '/share', data:shareData, timeout: 5000 }).success( function(response) {
	            callback(shareServer + response);
	        }).error(function(err){
	            callback("http://www.meteoearth.com/img/me_splash.jpg");   
	        });
	    };

		$scope.followFacebook = function() {
	        raiseGAEvent(GASocial.category, GASocial.action.follow, GASocial.label.network, GASocial.value.facebook);
	    };

	    $scope.followTwitter = function() {
	        raiseGAEvent(GASocial.category, GASocial.action.follow, GASocial.label.network, GASocial.value.twitter);
	    };

	    $scope.openSharePopup = function(target) {

	        raiseGAEvent(GASocial.category, GASocial.action.share, GASocial.label.network, target);

	        if(target === "facebook") {
	            FB.ui({
	               method: 'feed',
	               name: 'MeteoEarth.com',
	               description: (text.share_desc),
	               link: $scope.shareURL,
	               picture: $scope.shareImageUrl
	              },
	              function(response) {
	                if (response && response.post_id) {
	                    $scope.sendPostId("facebook", response.post_id); 
	                }
	              });
	        } else if(target === "twitter") {
	            var W=550,H=450,L=Math.round((screen.width - W) * 0.5),T=Math.round((screen.height - H) * 0.5);
	            window.open('http://twitter.com/intent/tweet/?text=%23MeteoEarth&url=' + encodeURIComponent($scope.shareURL),'',
	                        'left='+L+',top='+T+',width='+W+',height='+H+',personalbar=0,toolbar=0,scrollbars=1,resizable=1');
	        } else if(target === "gplus") {
	            var W=550,H=450,L=Math.round((screen.width - W) * 0.5),T=Math.round((screen.height - H) * 0.5);
	            window.open('https://plus.google.com/share?url=' + encodeURIComponent($scope.shareURL),'',
	                        'left='+L+',top='+T+',width='+W+',height='+H+',personalbar=0,toolbar=0,scrollbars=1,resizable=1');
	        }
	    };

		$scope.isPlayBackEnabled=false;
	    $scope.togglePlayback = function(enable) {
	        if(enable == undefined) {
	            $scope.isPlayBackEnabled = !$scope.isPlayBackEnabled;
	            if( $scope.isPlayBackEnabled ) {
	                raiseGAEvent(GAMisc.category, GAMisc.action.click, GAMisc.label.start_playback, "");
	            }
	        } else {
	            $scope.isPlayBackEnabled = enable;
	        }
	        if($scope.isPlayBackEnabled){
	            $scope.showAds();
	        }
	    ...
	    $scope.showModal = function(content_type) {
			$scope.modalShown = true;
			$scope.modal_content = "nav/" + content_type + "/" + $scope.navLang + ".html";
		};

		$scope.openMeteo24 = function() {
	        raiseGAEvent(GAPOI.category, GAPOI.action.click, GAPOI.label.meteo24, $scope.meteo24POIUrl);
	    };

		$scope.adsLoadedOnce = false;
	    $scope.hideAds = function() {
	        $scope.skyscraperAdsOffsetClass = null;
	        raiseGAEvent(GAMisc.category, GAMisc.action.click, GAMisc.label.close_ads, "");
	    }
	    $scope.showAds = function() {
	        var isFirstShow = !$scope.adsLoadedOnce;
	        if(isFirstShow) {
	            $scope.adsLoadedOnce = true;
	            loadGoogleAds();
	        }
	        if($scope.skyscraperAdsOffsetClass !== 'offset' && $(window).width() > 1015) {
	            $scope.skyscraperAdsOffsetClass = 'offset';
	            showGoogleAds(isFirstShow ? $scope.currentLang : undefined);
	        }
	    };

	    $scope.stopsocialPOIMainLoadSpinner = function() {
	        usSpinnerService.stop("socialPOIMainLoadSpinner");
	    };

	    $scope.stopsocialPOIUserLoadSpinner = function() {
	        usSpinnerService.stop("socialPOIUserLoadSpinner");
	    };

	    $scope.toggle3D = function($event) {
	    	var button = $event.currentTarget;
            var isEnabled = (button.className.indexOf("_on") >= 0);
            /* change button class handled in on3DEnabledChanged() */
            // change 3d
            meteoEarth.set3DEnabled(!isEnabled);
            raiseGAEvent(GALayer.category, isEnabled ? GALayer.action.enable : GALayer.action.disable, GALayer.label.misc, GALayer.value._3D);
      	};
      	$scope.toggleLighting = function($event) {
			var button = $event.currentTarget;
            var isEnabled = (button.className.indexOf("_on") >= 0);
            /* change button class handled in onLightingEnabledChanged() */
            // change 3d
            meteoEarth.setLightingEnabled(!isEnabled);
			raiseGAEvent(GALayer.category, isEnabled ? GALayer.action.enable : GALayer.action.disable, GALayer.label.misc, GALayer.value.lighting);
		};
	    ...
	    $scope.toggleLayer = function(index) {
	    	...
	    	$scope.disableOverlays();

	    	var active = ($scope.layerButtonClass[index] == 'active');

	    	raiseGAEvent(GALayer.category, active ? GALayer.action.disable : GALayer.action.enable, GALayer.label.layer, GALayer.value.layers[index]);

		    // main feature
		   	meteoEarth.setLayerEnabled(index, !active);

		   	/* 
		   	 * Button class change in setButtonClass
		   	 * triggered by onLayerEnabledChanged/onSocialWeatherEnabled/onVOREnabled/onTropicalStormEnabled
		   	 * in setLayerEnabled (me_engine.js)
		   	 */

		   	// additional features depending on layer
			if (index == 5) {
	    		/* storm layer */

	    		$timeout(function () {
				  $scope.showStormList = !active;
				});

				meConfig.checkPickerInfoAvailability();

				if(!active && !meConfig.isAnyLayerEnabled()) {
				  // if not layer is active, also enable wind to visualize storm
				  meteoEarth.setLayerEnabled(2, !active);
				  $scope.layerButtonClass[2] = 'active';
				}

	    	} else if (index == 7) {
	    		/* volvo ocean race layer */

    			if(active) {
					meConfig.removePOI("vorPOI");
				}
				$scope.generateCodeConditionally();
	    	};        
	    };

	    $scope.closeSocialPOI = function() {
	        window.document.getElementById("social_poi_video_player").pause();
	        $timeout(function () {
	         	$scope.showSocialPoi = false;
	        });
	    }

	    $scope.performZoomIn = function(performZoomIn) {
	        meConfig.performZoomIn = performZoomIn;
	    };
	    $scope.performZoomOut = function(performZoomOut) {
	        meConfig.performZoomOut = performZoomOut;
	    };

	    $scope.focusStorm = function(storm) {
	        meteoEarth.centerOnLocation(storm.lon, storm.lat);
	    };

	    $scope.generateCodeConditionally = function() {
	        if($scope.showEmbedWindow) { 
	            $timeout(function() {
	                $scope.generateCode();
	            }); 
	        }
	     ...
	     $scope.generateCode = function() {
	        var htmlCode =
	            ...
	            "<div id='meDiv' style='width:" + $scope.embedWidth.val + "px;height:" + $scope.embedHeight.val + "px;background:#000;position:relative'></div>" +
	            "<script type='text/javascript'>" +
	            "var meConfig={div:document.getElementById('meDiv')," +
	            "viewSettings:{longitude: " + meConfig.viewSettings.longitude.toFixed(4) + 
	            ",latitude: " + meConfig.viewSettings.latitude.toFixed(4) + 
	            ",zoom:" + meConfig.viewSettings.zoom.toFixed(4) + 
	            ",is3DEnabled:" + meConfig.viewSettings.is3DEnabled + 
	            ",isLightingEnabled:" + meConfig.viewSettings.isLightingEnabled;

	        if( meConfig.viewSettings.isTemperatureEnabled ) { htmlCode += ", isTemperatureEnabled:true" };
	        if( meConfig.viewSettings.isPrecipitationEnabled ) { htmlCode += ", isPrecipitationEnabled:true" };
	        if( meConfig.viewSettings.isWindEnabled ) { htmlCode += ", isWindEnabled:true" };
	        if( meConfig.viewSettings.isCloudEnabled ) { htmlCode += ", isCloudEnabled:true" };
	        if( meConfig.viewSettings.isPressureEnabled ) { htmlCode += ", isPressureEnabled:true" };
	        if( meConfig.viewSettings.isTropicalStormEnabled ) { htmlCode += ", isTropicalStormEnabled:true" };
	        if( meConfig.viewSettings.isVOREnabled ) { htmlCode += ", isVOREnabled:true" };
	        if( meConfig.viewSettings.initialRotationSpeed !== 0.0 ) { htmlCode += ", initialRotationSpeed:" + meConfig.viewSettings.initialRotationSpeed };

	        htmlCode += "}};me_createView(meConfig);</script>";
...
$( window ).resize(function() {
	checkScrollable($('.navbar-left'));
});

function init() {

	// start phone view with closed navigation
	if ($(window).width() <= 450) {
    	$('.navbar-left').removeClass('open');
    };

	$navbarItems = $('.navbar-left .navbar-items > li');
	...
	/* Navbar Items */
	// $('a', $navbarItems).click(function() {
	// 	toggleNavbarMenuItem($(this).parent());	
	// })


	/* Tooltip Menus */
	// $('.tooltip-menu .trigger').click(function() {
	// 	toggleTooltipMenu($(this).parent());
	// });

	checkScrollable($('.navbar-left'));

	// Rainbow Social Bar on Phone
	$(".social-like-container-phone .social-like li").click(function() {
...
/* Add scrollable class to navbar for width adjustments */
function checkScrollable($navbar) {

	hasVerticalScrollbar = $('ul.navbar-items', $navbar).get(0).scrollHeight > $('ul.navbar-items', $navbar).get(0).clientHeight;
...
/* Toggle Navbar */
function toggleNavbar($navbarToggle) {
	$navbarToggle.parent().parent().toggleClass('open');
}

/* Toggle Navbar Menu Item */
function toggleNavbarMenuItem($navbarItem) {

	$thisNavbarItem = $navbarItem;
	// $thisInfoSlide = $('.info-slide', $thisNavbarItem);
	// $infoSlides = $('.info-slide', $navbarItems);
	// $isActive = $thisNavbarItem.hasClass('active');

	// toggle shown class if navbar item is not active yet OR info slide is already shown
	// if (!$isActive || $thisInfoSlide.hasClass('shown')) {
	// 	$thisInfoSlide.toggleClass('shown');
	// }

	// show/hide info slides
	// $infoSlides.not($thisInfoSlide).removeClass('shown');

	// active class for navbar elements
	$thisNavbarItem.toggleClass('active');
}

/* Toggle Tooltip Menu */
function toggleTooltipMenu($menu){
	$this = $menu;

	$thisContent = $this.find('.content');
	$thisShown = $thisContent.hasClass('shown');
	$allContent = $('.tooltip-menu .content');


	$allContent.removeClass('shown');
	if (!$thisShown) {
		$thisContent.addClass('shown');
	};
}
...
(function(){
	...
	if ( /(iPad).*AppleWebKit.*Mobile.*Safari/.test(navigator.userAgent) ) {
      ...
      sbNode.content = 'app-id=586878855';
      headNode.appendChild(sbNode);
    } else if ( /(iPhone|iPod).*AppleWebKit.*Mobile.*Safari/.test(navigator.userAgent) ) {
      var headNode = document.getElementsByTagName("head")[0];
      var sbNode = document.createElement('meta');                    
      sbNode.name = 'apple-itunes-app';                
      sbNode.content = 'app-id=619814752';
      headNode.appendChild(sbNode);
    }
    ...
    if ( navigator.userAgent.match(/Android/i) != null ) {
	    $.smartbanner({
			title: MeteoEarth, // What the title of the app should be in the banner (defaults to <title>)
			author: MeteoGroup // What the author of the app should be in the banner (defaults to <meta name="author"> or hostname)
		});
	}
